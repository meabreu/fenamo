DROP FUNCTION fenamosq.start_session_sys(character varying, character varying, character varying, timestamp without time zone, numeric, character varying);

CREATE OR REPLACE FUNCTION fenamosq.start_session_sys(
    IN pi_cod_user character varying,
    IN pi_password character varying,
...
$BODY$
  ...
  BEGIN 
    ...
    po_cod_err := 'SESSION-004';
    SELECT rs 
      INTO v_pass_in
      FROM encrypt(pi_password::bytea,'1234'::bytea,'aes'::text) rs;

    po_cod_err := 'SESSION-005';
    SELECT passwd
      INTO v_pass_register
      FROM fenamosq.emp_user
     WHERE cod_user = pi_cod_user;



     IF v_pass_register IS NULL THEN 
         po_cod_err := 'SESSION-006';
         RAISE EXCEPTION 'CONDICTION';

     ELSIF v_pass_register = v_pass_in THEN
        
        po_cod_err := 'SESSION-007';
        INSERT INTO fenamosq.emp_session(
                    cod_user, state, tran_date_start, tran_id_start, tran_user_start)
            VALUES (pi_cod_user, 'A', v_tran_date, v_tran_id, v_tran_user);

        po_cod_err := 'SESSION-008';
        ...
        WHERE cod_role = po_cod_role;
          ...
          po_cod_user := pi_cod_user;
          po_cod_err := 'OK';
          po_tran_id := v_tran_id;
     ELSE 
        po_cod_err := 'SESSION-009';
        SELECT rs.po_tran_id,
               rs.po_cod_err,
               rs.po_msj_err,
               rs.po_msj_err_pl
          INTO po_tran_id,
               ...
               po_msj_err,
               po_msj_err_pl
          FROM fenamosq.ses_emp_user_fail(
  ...
  COST 100;
ALTER FUNCTION fenamosq.start_session_sys(character varying, character varying, character varying, timestamp without time zone, numeric, character varying)
  OWNER TO postgres;
