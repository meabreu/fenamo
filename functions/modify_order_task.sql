DROP FUNCTION fenamosq.modify_order_task(bigint, character varying, character varying, integer, character varying, character varying, timestamp without time zone, numeric, character varying);

CREATE OR REPLACE FUNCTION fenamosq.modify_order_task(
    IN pi_id_order_task bigint,
    IN pi_cod_user character varying,
    IN pi_state character varying,
    IN pi_worked integer,
    IN pi_observation character varying,
...
$BODY$
  ...
  BEGIN 
      ...
      IF pi_state = 'FINISH' THEN 
        v_end_date = v_tran_date;
      ELSE
        v_end_date = null;
      END IF;
      ...
      IF v_rec_ord_order_task.quant_completed::numeric + pi_worked::numeric = v_rec_ord_order.quant_order THEN 
        v_end_date = v_tran_date;
        v_task_state := 'FINISH';
      ELSE 
        v_end_date = NULL;
        v_task_state := pi_state;
      END IF;

      po_cod_err := 'MOD-ORDER-TASK-005';
      UPDATE fenamosq.ord_order_task
         SET quant_completed = (quant_completed::numeric + pi_worked::numeric),
             quant_worked = pi_worked::numeric,
             date_started = (CASE 
                                WHEN date_started IS NULL THEN v_tran_date 
                                ELSE date_started END
                            ),
             time_waited  = (CASE 
                                WHEN task_state = 'CREATED' THEN EXTRACT(EPOCH FROM (NOW() - tran_date)) / 60  
                                ELSE time_waited END
                            ),
             time_worked  = (CASE 
                                WHEN task_state = 'IN_PROCCESS' THEN (EXTRACT(EPOCH FROM (NOW() - tran_date)) / 60 ) + time_worked 
                                ELSE time_worked END
                            ),
             time_paused  = (CASE 
                                WHEN task_state = 'PAUSED' THEN (EXTRACT(EPOCH FROM (NOW() - tran_date)) / 60 ) + time_paused 
                                ELSE time_paused END
                            ),
             date_finished = v_end_date,
             task_state = v_task_state,
             user_asign = pi_cod_user,
             tran_user = pi_tran_user,
             observation = pi_observation,
  ...
  COST 100;
ALTER FUNCTION fenamosq.modify_order_task(bigint, character varying, character varying, integer, character varying, character varying, timestamp without time zone, numeric, character varying)
  OWNER TO fenamo;
